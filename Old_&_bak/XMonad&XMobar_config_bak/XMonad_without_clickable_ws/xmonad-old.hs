...
myBrowser            = "opera-developer"
myTerminal           = "urxvtc -name term"
myShell              = "zsh"
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((mod1Mask,               0x1008ff5d),     spawn "gksu pcmanfm")
    ...
    , ((0,                          0xffc9),     spawn "urxvtc -name terminal")
    ...
    , ((mod1Mask,                     0x66),     spawn "pavucontrol")                                                                                 --Alt+F
	...
	-- , ((mod1Mask,                     0x6e),     spawn "xnvim")                                                                                       --Alt+N
    ...
    --, ((modm     .|. shiftMask,       0x70),     spawn "dmenu_run -i -p 'Run:' -sb '#333' -nf '#999' -sf '#9df' -fn 'Terminus Re33:size=12'")         --Win+Shift+P
...
myLayout =  avoidStruts
            ...
            $ smartBorders
            $ onWorkspace "W"  (Full  ||| tiled ||| Mirror tiled)
            $ onWorkspace "M"  (tiled ||| centerMaster Grid)
            $ onWorkspace "E"  (tiled ||| Mirror tiled ||| GridRatio 1) 
            $ onWorkspace "F"  (tiled ||| (TwoPane 0.01 0.5) ||| (Mirror $ TwoPane 0.01 0.5))
            $ onWorkspace "S"  (tiled ||| Mirror tiled)
            $ onWorkspace "V"  (Full  ||| Grid)
...
myManageHook = composeAll . concat $
	...
	-- , [stringProperty "WM_WINDOW_ROLE" =? "page-info"   --> doCenterFloat]
    ...
    where
    myWeb     = ["Firefox","Chromium","Opera","Opera developer","Iron","Tor Browser"]
    myMail    = ["Thunderbird"]
...
myLogHook = dynamicLogString $ xmobarPP {
        ...
        , ppTitle           = xmobarColor "#999" "". shorten 100
        }
