Config { font = "xft:Terminus Re33:size=12:antialias=true:hinting=true"
       ...
       , commands = [ Run XMonadLog
                    ...
                    --, Run Com "fan1" [] "" 50
                    --, Run Com "fan2" [] "" 50
                    , Run Memory ["-t", "Mem:<fn=2> </fn><usedratio>%","-L","50","-H","90","--normal","yellow","--high","orange"] 50
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " %XMonadLog% }{ <<fc=#88bbdd><</fc><fc=#aaddff><</fc><fc=#88bbdd><</fc><  %root-disk%  %enp3s0%  <fc=red>↓</fc><fn=2> </fn>%Net_in% <fc=green>↑</fc><fn=2> </fn>%Net_out%  %cpu% %cpu-temp% %cpu-freq%MHz  MB:<fn=2> </fn>%mb-temp%  %memory%  Vol:<fn=2> </fn>%vol2.sh%  <fn=1>%kbd%</fn>  %xmdate% <fc=#aaddff>%xmtime%</fc>   <fn=1><fc=#aaddff>%weather%</fc></fn>"
       }
