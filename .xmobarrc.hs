Config { font = "xft:Terminus Re33:size=12:antialias=true:hinting=true"
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " %XMonadLog% }{ <<fc=#88bbdd><</fc><fc=#aaddff><</fc><fc=#88bbdd><</fc><  %enp3s0%  %Net_in%<fn=2> </fn><fc=green>↓</fc><fc=red>↑</fc><fn=2> </fn>%Net_out%  Σ:<fn=2> </fn>%Net_all%  %cpu% %TCPU%  MB:<fn=2> </fn>%TMB%  GPU:<fn=2> </fn>%TGPU%  %memory%  ROM:<fn=2> </fn>%ROM%  Up:<fn=2> </fn>%xmuptime%  Vol:<fn=2> </fn>%XVol%  <fn=1>%kbd%</fn>  %xmdate%  <fc=#aaddff>%xmtime%</fc>  <fn=1><fc=#aaddff>%weather%</fc></fn>"
       }
