...
-- StartupHook
myStartupHook  =  return () <+> adjustEventInput <+> setWMName "LG3D" <+> onScr 1 W.greedyView "W" <+> spawn "gis-weather" <+> spawn "echo > /tmp/haron/MW2" <+> spawn "echo > /tmp/haron/DO" <+> spawn "echo > /tmp/haron/NS" <+> spawn "echo > /tmp/haron/TR" <+> spawn "XMVol2" <+> spawn "XMUpdate" <+> spawn "XMRomNew" <+> spawn "XMMPD" <+> spawn "XMUptime" 
