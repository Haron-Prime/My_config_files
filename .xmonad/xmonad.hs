...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((0,                      0x1008ff30),     spawn "if pgrep 'transgui' >/dev/null; then pkill transgui; else transgui; fi")
    ...
    , ((0,                      0x1008ff1d),     spawn "if pgrep 'galculator' >/dev/null; then pkill galculator; else galculator; fi")
    , ((0,                      0x1008ff2f),     spawn "i3lock -i /home/haron/wall/starrynight.png")
    , ((0,                      0x1008ff81),     spawn "if pgrep 'ncmpcpp' >/dev/null; then pkill ncmpcpp; else urxvtc -name ncmpcpp -e /usr/bin/ncmpcpp; fi")
    , ((0,                      0x1008ff1b),     spawn "if pgrep 'htop' >/dev/null; then pkill htop; else urxvtc -name htop -e /usr/bin/htop; fi")
