...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((0,                      0x1008ff30),     spawn "if pgrep -x 'transgui' >/dev/null; then kill -9 $(pgrep 'transgui'); else transgui; fi")
    ...
    , ((0,                      0x1008ff1d),     spawn "if pgrep -x 'galculator' >/dev/null; then kill -9 $(pgrep 'galculator'); else galculator; fi")
    , ((0,                      0x1008ff2f),     spawn "i3lock -i /home/haron/wall/starrynight.png")
    , ((0,                      0x1008ff81),     spawn "if pgrep -x 'ncmpcpp' >/dev/null; then kill -9 $(pgrep 'ncmpcpp'); else urxvtc -name ncmpcpp -e /usr/bin/ncmpcpp; fi")
    , ((0,                      0x1008ff1b),     spawn "if pgrep -x 'htop' >/dev/null; then kill -9 $(pgrep 'htop'); else urxvtc -name htop -e /usr/bin/htop; fi")
