...
myLogHook = do
            dynamicLogString $ xmobarPP {
                                        ...
                                        , ppUrgent          = xmobarColor myUrgColor "" . wrap "[" "]"
                                        , ppOrder           = \(ws:_:t:_) -> [ws]
                                        }

-- StartupHook
myStartupHook  =  return () <+> adjustEventInput <+> setWMName "LG3D" <+> onScr 1 W.greedyView "W" <+> spawn "echo > /tmp/haron/MW2" <+> spawn "gis-weather" <+> spawn "XMVol2" <+> spawn "XMUpdate" <+> spawn "XMRomNew" <+> spawn "XMMPD" <+> spawn "XMTraf" <+> spawn "XMTrans" <+> spawn "XMUptime"
