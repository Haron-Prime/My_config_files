...
myWS = clickable . (map xmobarEscape) $ [ "W", "M", "E", "F", "S", "V", "P", "J", "T" , "X" , "XI" , "XII"]
    where clickable l = [ "<action=`xdotool key 0xffeb+" ++ show (n) ++ "` button=1><action=`xdotool key 0xffeb+0xff1b` button=3><action=`xdotool key 0xffe3+0xffe9+0xff51` button=4><action=`xdotool key 0xffe3+0xffe9+0xff53` button=5><fn=4>" ++ ws ++ "</fn></action></action></action></action>" |
...
main = do
    xmproc <- spawnPipe "xmobar $HOME/.xmonad/xmobar-test.hs"
