...
Config {
       ...
       , sepChar  = "%"
       , alignSep = "}{"
       , template = "<fn=2> </fn><action=`mygtkmenu .menurc` button=1><action=`mygtkmenu .bookmarksrc` button=3><icon=arch-mono-16x16.xpm/></action></action> <action=`xdotool key Super_L+Escape`>  </action><action=`mygtkmenu .workspacesrc` button=13><action=`xdotool key Control_L+Alt_L+Left` button=4><action=`xdotool key Control_L+Alt_L+Right` button=5>%XMonadLog%</action></action></action><action=`xdotool key Super_L+grave`>  </action>%MW%}{<action=XMYaourt>%Update%%XMUpdate%</action>  <action=`XMncmpcpp` button=3><action=`mpc prev` button=1><icon=backward.16x16.xpm/></action><fn=2> </fn><action=`mpc toggle` button=1><icon=pause.16x16.xpm/><icon=start.16x16.xpm/></action><fn=2> </fn><action=`mpc next` button=1><icon=forward.16x16.xpm/></action></action>   <action=`XMVnstat-h` button=1><action=`XMVnstat-d` button=3>%dynnetwork%</action></action>  <action=`XMVnstat` button=1><action=`XMVnstat-m` button=3>%XMTraf%</action></action>  <action=`XMTop-cpu` button=1><action=`XMHtop` button=3>%multicpu% %XMTCpu%</action></action>  <action=`XMSensors` button=13>%XMTMB%  %XMTGpu%</action>  <action=`XMTop-mem` button=1>%memory%</action>  <action=`XMFree` button=1>%swap%</action>  <action=`XMdf-h` button=1><action=`XMblkid` button=3>%XMRom%</action></action>  %XMUptime%  <action=`pulseaudio-ctl up` button=5><action=`pulseaudio-ctl down` button=4><action=`pulseaudio-ctl mute` button=1><action=`XMPavucontrol` button=3>%XMVol%</action></action></action></action>  %kbd%  <action=`XMGSimplecal` button=1><action=`XMCal` button=3>%XMTime%</action></action><action=`xdotool key Super_L+0x63`>  </action><action=`XMAccuWeather` button=1><action=`XMGismeteo` button=3><action=`GWeather` button=45><fc=#95d5f5><fn=1>%Cur_temp%</fn></fc></action></action></action><action=`xdotool key Super_L+0x71`>  </action>"
       }
