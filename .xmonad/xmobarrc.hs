...
Config {
       ...
       , sepChar  = "%"
       , alignSep = "||"
       , template = "<fn=2> </fn><action=`XMScreenfetch` button=13><icon=arch-mono-16x16.xpm/></action> <action=`mygtkmenu .menurc` button=13><fn=3></fn></action> <action=`xdotool key 0xffeb+0xff1b` button=1><action=`mygtkmenu .workspacesrc` button=3><action=`xdotool key 0xffe3+0xffe9+0xff51` button=4><action=`xdotool key 0xffe3+0xffe9+0xff53` button=5>%XMonadLog%</action></action></action></action> <action=`mygtkmenu .bookmarksrc` button=13><fn=3></fn></action>  %MW2%||<action=XMYaourt>%UP%%XMUpdate%</action><fn=5>  </fn><action=`XMVnstat-h` button=1><action=`XMVnstat-d` button=3>%dynnetwork%</action></action> <action=`xdotool key 0xffeb+0x39` button=1><action=`xdg-open http://localhost:9091/transmission/web/` button=2><action=`XMTransgui` button=3>%XMTrans%</action></action></action> <action=`XMVnstat` button=1><action=`XMVnstat-m` button=3>%XMTraf%</action></action><fn=5>  </fn><action=`XMTop-cpu` button=1><action=`XMHtop` button=3>%multicpu%</action></action> <action=`XMSensors` button=13>%XMTCpu%<fn=5>  </fn>%XMTMB%<fn=5>  </fn>%XMTGpu%</action><fn=5>  </fn><action=`XMTop-mem` button=13>%memory%</action> <action=`XMFree` button=13>%swap%</action> <action=`XMdf-h` button=1><action=`XMblkid` button=3>%XMRom%</action></action><fn=5>  </fn><action=`XMHtop` button=1><action=`XMinxi` button=3>%XMUptime%</action></action><fn=5>  </fn><action=`XMVolUp` button=5><action=`XMVolDown` button=4><action=`XMVolMute` button=1><action=`XMPavucontrol` button=3>%VO%</action></action></action></action><fn=5>  </fn><action=`XMMPC` button=45><action=`XMncmpcpp` button=3><action=`mpc prev` button=1><fn=3></fn></action> <action=`mpc toggle` button=1><action=`mpc stop` button=2>%XMMPD%</action></action> <action=`mpc next` button=1><fn=3></fn></action></action></action><fn=5>  </fn><action=`xkblayout-state set +1`>%kbd%</action><fn=5>  </fn><action=`XMGSimplecal` button=1><action=`XMCal` button=3>%XMTime%</action></action><fn=5>  </fn><action=`GWeather` button=13>%WI%</action> <action=`XMAccuWeather` button=1><action=`XMGismeteo` button=3><fc=#95d5f5><fn=1>%CT%</fn></fc></action></action><action=`XMXClock` button=13> <fc=#ccc><fn=3></fn></fc> </action>"
       }
