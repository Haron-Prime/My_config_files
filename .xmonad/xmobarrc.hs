...
Config {
       ...
       , sepChar  = "%"
       , alignSep = "}{"
       , template = "<action=`i3lock -i /home/haron/wall/starrynight.png` button=1><action=`compdown` button=2><action=`compreboot` button=3><icon=arch-mono-16x16.xpm/>%XMNull%</action></action></action> %XMonadLog%}{<action=urxvtc -name update -e yaourt -Syua>%XMUpdate%</action>  <action=`XMVnstat-h` button=1><action=`XMVnstat-d` button=3>%dynnetwork%</action></action>  <action=`XMVnstat` button=1><action=`XMVnstat-m` button=3>%XMTraf%</action></action>  <action=`XMTop-cpu` button=1><action=`XMHtop` button=3>%cpu% %TCPU%</action></action>  <action=XMSensors>%TMB%  %TGPU%</action>  <action=`XMTop-mem` button=1><action=`XMFree` button=3>%memory%</action></action>  <action=`XMdf-h` button=1><action=`XMblkid` button=3>%XMRom%</action></action>  <action=`XMLast` button=1>%XMUptime%</action>  <action=`/usr/bin/pulseaudio-ctl up` button=5><action=`/usr/bin/pulseaudio-ctl down` button=4><action=`/usr/bin/pulseaudio-ctl mute` button=1><action=`XMncmpcpp` button=2><action=`mpc toggle` button=3>%XMVol%</action></action></action></action></action>  <action=`xdotool key Mode_switch` button=1345>%kbd%</action>  <action=`XMCal` button=1><action=`XMXClock` button=3>%XMTime%</action></action>  <action=`XMWeather-current` button=1>%XMWeather%</action>"
       }
