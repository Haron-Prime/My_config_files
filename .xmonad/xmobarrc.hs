...
Config {
       ...
       , additionalFonts  = [
                            ...
                            ,"xft:Reactor Sans:size=8:weight=bold:antialias=true:hinting=true"
       ...
       , sepChar  = "%"
       , alignSep = "||"
       , template = " <action=`XMScreenfetch` button=3><action=`mygtkmenu .menurc` button=1><fn=6></fn></action></action> <action=`xdotool key 0xffeb+0xff1b` button=1><action=`mygtkmenu .workspacesrc` button=3><action=`xdotool key 0xffe3+0xffe9+0xff51` button=4><action=`xdotool key 0xffe3+0xffe9+0xff53` button=5><fn=4>%XMonadLog%</fn></action></action></action></action><fn=2>  </fn><action=`mygtkmenu .bookmarksrc` button=13><fn=6></fn></action> %MW2%||<action=XMYaourt>%UP%</action> <action=`XMVnstat-h` button=1><action=`XMVnstat-d` button=3>%dynnetwork%</action></action> <action=`xdg-open http://localhost:9091/transmission/web/` button=3><action=`XMTransgui` button=1>%TU%</action></action> <action=`XMVnstat` button=1><action=`XMVnstat-m` button=3>%TR%</action></action>  <action=`XMTop-cpu` button=1><action=`XMHtop` button=3><fc=#ccc></fc><fn=2> </fn>%CU%</action></action>  <action=`XMSensors` button=13><fc=#ccc></fc><fn=2> </fn>%TEMP%</action>  <action=`XMTop-mem` button=1><action=`XMFree` button=3><fc=#ccc><fn=6></fn></fc><fn=2> </fn>%MEM%</action></action> <action=`XMdf-h` button=1><action=`XMblkid` button=3>%RU%</action></action>  <action=`XMHtop` button=1><action=`XMinxi` button=3>%UT%</action></action>  <action=`XMVolUp` button=5><action=`XMVolDown` button=4><action=`XMVolMute` button=1><action=`XMPavucontrol` button=3>%VO%</action></action></action></action>  <fn=6><action=`XMMPC` button=45><action=`XMncmpcpp` button=3><action=`XMMPCprev` button=1></action> <action=`XMMPCtoggle` button=1><action=`XMMPCstop` button=2>%PS%</action></action> <action=`XMMPCnext` button=1></action></action></action></fn>  <action=`xkblayout-state set +1`>%kbd%</action>  <action=`XMGSimplecal` button=1><action=`XMCal` button=3>%XMTime%</action></action>  <action=`XMGWeather` button=13>%WI%</action><fn=2> </fn><action=`XMAccuWeather` button=1><action=`XMGismeteo` button=3><fc=#95d5f5><fn=1>%CT%C</fn></fc></action></action><action=`oblogout` button=13> <fc=#ccc></fc> </action>"
       }
