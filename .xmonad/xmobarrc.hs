...
Config {
       ...
       , sepChar  = "%"
       , alignSep = "}{"
       , template = "<fn=2> </fn><action=`xdotool key 0xffeb+0xff1b` button=13><icon=arch-mono-16x16.xpm/></action> <action=`mygtkmenu .menurc` button=1><action=`mygtkmenu .bookmarksrc` button=3><fn=3></fn></action></action> <action=`mygtkmenu .workspacesrc` button=13><action=`xdotool key 0xffe3+0xffe9+0xff51` button=4><action=`xdotool key 0xffe3+0xffe9+0xff53` button=5>%XMonadLog%</action></action></action> <action=`xdotool key 0xffeb+0x60` button=13><fn=3></fn></action>  %MW%}{<action=XMYaourt>%Update%%XMUpdate%</action>  <action=`XMVnstat-h` button=1><action=`XMVnstat-d` button=3>%dynnetwork%</action></action> <action=`xdotool key 0xffeb+0x39` button=1><action=`xdg-open http://localhost:9091/transmission/web/` button=2><action=`XMTransgui` button=3>%XMTrans%</action></action></action> <action=`XMVnstat` button=1><action=`XMVnstat-m` button=3>%XMTraf%</action></action>  <action=`XMTop-cpu` button=1><action=`XMHtop` button=3>%multicpu%</action></action> <action=`XMSensors` button=13>%XMTCpu%  %XMTMB%  %XMTGpu%</action>  <action=`XMTop-mem` button=13>%memory%</action> <action=`XMFree` button=13>%swap%</action>  <action=`XMdf-h` button=1><action=`XMblkid` button=3>%XMRom%</action></action>  <action=`XMHtop` button=1><action=`XMinxi` button=3>%XMUptime%</action></action>  <action=`pulseaudio-ctl up` button=5><action=`pulseaudio-ctl down` button=4><action=`pulseaudio-ctl mute` button=1><action=`XMPavucontrol` button=3>%XMVol%</action></action></action></action>  <action=`XMMPC` button=45><action=`XMncmpcpp` button=3><action=`mpc prev` button=1><fn=3></fn></action> <action=`mpc toggle` button=1><action=`mpc stop` button=2>%XMMPD%</action></action> <action=`mpc next` button=1><fn=3></fn></action></action></action>  <action=`xkblayout-state set +1`>%kbd%</action>  <action=`XMGSimplecal` button=1><action=`XMCal` button=3>%XMTime%</action></action>  <action=`GWeather` button=13><fc=#cccccc>%XMWeatherIcon%</fc></action><fn=2> </fn><action=`XMAccuWeather` button=1><action=`XMGismeteo` button=3><fc=#95d5f5><fn=1>%CT%</fn></fc></action></action> <action=`killall dropbox` button=3>%XMDropbox%</action><action=`xdotool key 0xffeb+0x62` button=13><fc=#cccccc><fn=3></fn></fc></action> "
       }
